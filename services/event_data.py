...
def agent_avail_select(timestamp):
    ...
    try:
        with connection.cursor() as cursor:
            sql = "SELECT name FROM mon_app_agentsystem WHERE timestamp<" + timestamp
            cursor.execute(sql)
            result = cursor.fetchall()
            return result
    finally:
...
def agent_avail_event_open(timestamp, name, message, severity):
    ...
    try:
        with connection.cursor() as cursor:
            sql = r"""INSERT INTO mon_app_agentevent 
            ...
            WHERE NOT EXISTS (SELECT name FROM mon_app_agentevent WHERE name='""" + name + """' AND monitor='perf.system.availability.seconds' AND status=1)"""
            cursor.execute(sql)
            connection.commit()
    finally:
        connection.close()        
...
def agent_avail_select_event_open(timestamp):
    ...
    try:
        with connection.cursor() as cursor:
            sql = r"""SELECT DISTINCT t1.name FROM mon_app_agentevent as t1 
            INNER JOIN mon_app_agentdata as t2 on t1.name = t2.name
            ...
            if not result is None:
                for i in result:
                    name = i['name']
                    print(name)
                    sql = r"UPDATE mon_app_agentevent SET status=0 WHERE name='" + name + "'"
                    cursor.execute(sql)
                    connection.commit()
    finally:
        connection.close()
